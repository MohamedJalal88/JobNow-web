...
import {
  ArrowLeft, Bell, Building2, ChevronRight, CreditCard, Globe, HelpCircle, KeyRound, Languages,
  Lock, Mail, Moon, Trash2, User,
} from "lucide-react";
...
export const Route = createFileRoute("/contractor/settings")({
  head: () => ({ meta: [{ title: "Settings — JobNow Contractor" }] }),
  ...
  return (
    <main className="min-h-dvh bg-muted/30">
      <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 pt-6 md:pt-8 pb-12">
        <Link to="/contractor" className="inline-flex items-center gap-1 text-sm text-muted-foreground"><ArrowLeft className="h-4 w-4" /> Back to dashboard</Link>
        <h1 className="mt-4 text-2xl md:text-3xl font-extrabold tracking-tight">{t("Contractor Settings")}</h1>
        <p className="text-sm text-muted-foreground mt-1">{t("Manage your business account, preferences and security.")}</p>
        ...
        <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <Section title="Business Profile">
            <Row icon={Building2} label="Business details" desc="Company info, hiring preferences" to="/contractor/profile" chevron />
          </Section>

          <Section title={t("Account")}>
            <Row icon={Mail} label={t("Email address")} right={<span className="text-xs text-muted-foreground">{user?.email || "No email"}</span>} />
            <Row icon={KeyRound} label={t("Change password")} to="/contractor/change-password" chevron />
            <Row icon={CreditCard} label={t("Billing & Subscription")} desc={t("Manage billing details and checkouts")} to="/contractor/payments" chevron />
          </Section>
          ...
          <Section title={t("Security & privacy")}>
            <Row icon={Lock} label={t("Privacy & data")} to="/contractor/privacy" chevron />
            <Row icon={HelpCircle} label={t("Help & support")} to="/contractor/help" chevron />
        ...
        <p className="mt-8 text-center text-xs text-muted-foreground">JobNow Contractor Portal v1.0.0</p>
      </div>
    </main>
  );
}
