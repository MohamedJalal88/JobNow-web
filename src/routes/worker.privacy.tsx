...
import { ArrowLeft, Cookie, Database, Download, FileText, ShieldAlert, Trash2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Switch } from "@/components/ui/switch";
import { useState } from "react";
import { toast } from "sonner";

export const Route = createFileRoute("/worker/privacy")({
  head: () => ({ meta: [{ title: "Privacy & data — JobNow" }] }),
  component: Privacy,
});
...
function Privacy() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 md:px-6 lg:px-8 pt-6 md:pt-8">
      <Link to="/worker/settings" className="inline-flex items-center gap-1 text-sm text-muted-foreground"><ArrowLeft className="h-4 w-4" /> Back to settings</Link>

      <h1 className="mt-5 text-2xl md:text-3xl font-extrabold tracking-tight">Privacy & Data</h1>
      <p className="text-sm text-muted-foreground mt-1">Control how your data is used across JobNow.</p>

      <Section title="Data preferences" icon={Database}>
        <Toggle label="Personalised job recommendations" desc="Use your activity to suggest better jobs." value={prefs.analytics} onChange={(v) => setPrefs((p) => ({ ...p, analytics: v }))} />
        <Toggle label="Share location for nearby jobs" desc="Required to show jobs around you." value={prefs.location} onChange={(v) => setPrefs((p) => ({ ...p, location: v }))} />
        <Toggle label="Marketing communications" desc="Tips, offers and product updates." value={prefs.ads} onChange={(v) => setPrefs((p) => ({ ...p, ads: v }))} />
        <Toggle label="Share profile with partners" desc="Let trusted partner contractors discover you." value={prefs.share} onChange={(v) => setPrefs((p) => ({ ...p, share: v }))} />
      </Section>
      ...
      <Section title="Your data" icon={Download}>
        <Action title="Download my data" desc="Get a copy of everything we have about you." cta="Request" onClick={() => toast.success("We'll email your archive within 48h")} />
      ...
      <Section title="Danger zone" icon={ShieldAlert} tone="destructive">
        <Action danger title="Delete account" desc="Permanently remove your account and all associated data." cta="Delete" onClick={() => toast.error("Account deletion requested")} />
      </Section>
    </div>
  );
}
